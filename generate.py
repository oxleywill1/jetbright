...
@media(max-width:760px){
  ...
  .nav-links{display:none}                 /* links live in the footer + hero on mobile */
  .sms-long{display:none}                  /* button becomes just the number */
  .logo{font-size:20px}
  .sms-btn{font-size:14px;padding:10px 16px}
  .hero{padding:52px 0 48px}
  .hero::after{display:none}               /* diagonal streak crowds small screens */
  .hero h1{font-size:clamp(36px,11vw,52px);word-break:break-word}
  .hero p.lede{font-size:17px;margin:18px 0 26px}
  .hero-ctas{flex-direction:column;align-items:stretch}
  .hero-ctas a{text-align:center;width:100%}
  .btn-big,.btn-ghost{font-size:17px;padding:15px 22px}
  section{padding:52px 0}
  h2.display{font-size:clamp(26px,7.5vw,34px)}
  .svc{grid-template-columns:1fr;gap:14px}
  .ba{grid-template-columns:1fr}
  .ba .a{text-align:left}
  .quote-form{padding:22px}
  .ff input,.ff select{font-size:16px}      /* 16px stops iOS auto-zoom on focus */
  .form-split{gap:32px}
  .steps{gap:22px}
  ...
  .jobs-chips span{font-size:13px;padding:6px 13px}
  .side-card{padding:22px}
}
@media(max-width:380px){
  .sms-btn{font-size:13px;padding:9px 13px}
  .logo{font-size:18px}
}
"""
...
<meta name="apple-mobile-web-app-title" content="JetBright">
...
<nav class="nav" aria-label="Main">
  ...
  <a class="sms-btn" href="{SMS_A}"><span class="sms-long">Text a photo: </span>{PHONE}</a>
</nav>
</header>
...
# copy icons (favicon.svg, favicon-32.png, apple-touch-icon.png, icon-512.png)
import shutil
